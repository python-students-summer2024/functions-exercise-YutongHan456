...
def main():
    ...
    dice1 = roll_die()
    dice2 = roll_die()
    game_type = get_question_type()
    print_question(dice1,dice2,game_type)
    answer = input_answer()
    if answer == -1:
        print_error_message()
        return
    if is_correct_answer(dice1,dice2,game_type,answer) == True:
        print_congratulations(game_type)
    else:
        print_correct_answer(dice1,dice2,game_type)
